import { LevelWithDelta, Level, getTranslatedLevelTags } from "./level.js";
...
export const RankRates = Object.freeze({
    getAvg(rates: RankRates): number {
        return (
            sum(rates.map((value, index) => value * (index + 1))) / sum(rates)
        );
    },
    ...
    estimateStableLevel(metadata: PlayerMetadata, mode: GameMode): string {
        ...
        for (;;) {
            ...
            if (expectedGamePoint >= 0) {
                ...
                if (!level.isAllowedMode(mode) || level === lastPositiveLevel) {
                    return `${lastPositiveLevel.getTag()}+ (${expectedGamePoint.toFixed(
                        2,
                    )})`;
                }
            } else {
                if (lastPositiveLevel) {
                    return `${lastPositiveLevel.getTag()} (${calcPoint(
                        lastPositiveLevel,
                    ).toFixed(2)})`;
                }
                break;
        ...
        for (;;) {
            ...
            if (expectedGamePoint > -0.001) {
                return `${level.getTag()} (${Math.abs(calcPoint(level)).toFixed(
                    2,
                )})`;
            }
        }
