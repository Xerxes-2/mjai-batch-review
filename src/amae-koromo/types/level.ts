...
import { PLAYER_RANKS } from "./constants.js";
...
export function getTranslatedLevelTags(): string[] {
    const rawTags = PLAYER_RANKS as string;
...
export class Level {
    ...
    toLevelId() {
        return (
            this._numPlayerId * 10000 + this._majorRank * 100 + this._minorRank
        );
    }
    ...
    getTag(): string {
        const label =
            getTranslatedLevelTags()[
                this.isKonten() ? LEVEL_KONTEN - 2 : this._majorRank - 1
            ];
    ...
    getMaxPoint(): number {
        ...
        return LEVEL_MAX_POINTS[
            (this._majorRank - 1) * 3 + this._minorRank - 1
        ];
    }
    ...
    formatAdjustedScore(score: number) {
        const level = this.getAdjustedLevel(score);
        score = this.getVersionAdjustedScore(score);
        return `${level.getScoreDisplay(
            level.isSame(this) ? Math.max(score, 0) : level.getStartingPoint(),
        )}${
            level.getMaxPoint()
...
export const LevelWithDelta = Object.freeze({
    format(obj: LevelWithDelta): string {
        return new Level(obj.id).formatAdjustedScoreWithTag(
            obj.score + obj.delta,
        );
    },
